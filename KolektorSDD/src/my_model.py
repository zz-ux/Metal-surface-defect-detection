...
from src.Features_fusion import FeatureResidualOriginalFusing
...
class U2Net(nn.Module):
    def __init__(self, cfg: dict, out_ch: int = 1):
        ...
        side_channel_list.reverse()
        self.decode_modules = nn.ModuleList(decode_list)
        '''需要更改的'''
        self.feature_fuse = FeatureResidualOriginalFusing(side_channel_list, out_ch)
    ...
    def forward(self, x: torch.Tensor):
        ...
        '''特征提取完毕，得到6个特征图（大小：1/1; 1/2; 1/4; 1/8; 1/16; 1/32;）'''
        return self.feature_fuse(decode_outputs, x_temp)
...
if __name__ == '__main__':
    ...
    model = u2net_full(out_ch=5).eval()
    result = model(x)
    print(result.shape)
